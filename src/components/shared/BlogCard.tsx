...
const BlogCard = ({ blog }: TBlogProp) => {
    ...
    return (
        <Link href={`/blog/${blog?._id}`}>
            <div className=" p-4 sm:p-6 rounded-2xl w-full max-w-md hover:shadow-lg transition-all duration-300">
                {/* Image */}
                ...
                <div className="mt-4 flex items-center gap-2 sm:gap-3">
                    <p className="text-white text-xs sm:text-sm">By Humayun Kabir</p>
                </div>
            </div>
        </Link>
    );
